...
VCP_LOOKBACK_WINDOWS = [80, 130, 220]  # 3 key windows (was 9) — 3x faster
# Equivalent windows for weekly bars (divide by ~5)
VCP_WEEKLY_LOOKBACK_WINDOWS  = [16, 26, 44]  # 3 key windows
# Equivalent windows for monthly bars (divide by ~21)
VCP_MONTHLY_LOOKBACK_WINDOWS = [8, 13, 22]   # 3 key windows

# ── Contraction finder ────────────────────────────────────────────────────
VCP_MIN_CONTRACTIONS  = 2
VCP_MAX_CONTRACTIONS  = 5     # cap at 5T — 6T is overextended (Minervini rarely discusses 5T+)
...
VCP_REGIME_HARD_GATE_BEAR  = True    # skip new entries entirely when regime == "bear"
                                      # Backtest showed 2007/2011/2013/2018/2019
                                      # (all bear years) had 0% win rate
...
# ── Scoring gates ─────────────────────────────────────────────────────────
VCP_MIN_QUALITY_SCORE = 55.0   # live scanner gate (lowered: regime multiplier shrinks scores by ~5%)
VCP_BACKTEST_MIN_SCORE  = 60.0  # backtest entry gate
VCP_WATCHLIST_MIN_SCORE = 40.0  # daily report watchlist tab (forming patterns)  # backtest entry gate (lower to capture more history)
...
VCP_WALK_STEP_DAYS       = 5    # weekly step for backtest (was 1 — too slow, ~16hrs for full universe)
