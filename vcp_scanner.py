...
from config import (
    ...
    SCORE_THRESHOLDS, VCP_MIN_QUALITY_SCORE, VCP_RS_MIN,
    VCP_BUY_ZONE_PCT,
)
...
def scan_vcp(
    ...
    *rs_override* — cross-sectional RS Rating computed by main.py's
    ...
    if pattern is None:
        return None
    if pattern.quality_score < VCP_MIN_QUALITY_SCORE:
        log.debug("%s: VCP quality %.1f below threshold %.1f",
                  symbol, pattern.quality_score, VCP_MIN_QUALITY_SCORE)
        return None
    ...
    # 200-day EMA trend gate (same as C&H scanner)
    ...
    if rs < VCP_RS_MIN:
        log.debug("%s: RS %.1f below VCP minimum %d", symbol, rs, VCP_RS_MIN)
        return None
