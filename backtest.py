...
def backtest_vcp_symbol(
    ...
    for idx in range(VCP_MIN_BARS_BT, n, VCP_WALK_STEP_BT):
        ...
        # Compute indicators BEFORE detect_vcp so rs_rating is passed in
        try:
        ...
        try:
            pattern = detect_vcp(symbol, daily_slice, benchmark=benchmark, rs_rating=float(rs_val))
        except Exception as e:
        ...
        # Bear market hard gate — backtest showed 2007/2011/2013/2018/2019
        # (all confirmed bear regime years) had 0% win rate. Skip entries
        # entirely when the benchmark is in a bear regime at entry time.
        if VCP_REGIME_HARD_GATE_BEAR and pattern.market_regime == "bear":
            continue

        # Dedup key rounded to the week + nearest 10 on pivot price.
        # Multiple lookback windows on the same underlying base can produce
        # slightly different pivot prices for what is really the same trade —
        # without this rounding the same base gets entered multiple times.
        base_week = pattern.base_start_date - timedelta(
            days=pattern.base_start_date.weekday()
        )
        ...
        trade_log.append({
            ...
            "sepa_score":           round(pattern.sepa_score, 3),
            "t_count":              pattern.t_count,
            "window_strategy":      pattern.window_strategy,
            "base_start_date":      (
            ...
            "active_timeframes":    str(pattern.active_timeframes),
            "tf_combo_bonus":       round(pattern.tf_combo_bonus, 3),
            "vol_compression_score": round(pattern.vol_compression_score, 3),
            "volume_trend_score":   round(pattern.volume_trend_score, 3),
            "accumulation_score":   round(pattern.accumulation_score, 3),
        })
...
def backtest_universe_vcp(symbols: list[str], notes: str = "") -> dict:
    ...
    # Auto-generate Excel report after every successful universe backtest
    if summary.get("total_trades", 0) > 0:
        try:
            from backtest_vcp_report import generate_vcp_report
            paths = generate_vcp_report(run_id)
            for p in paths:
                log.info("Backtest Excel report saved → %s", p)
        except Exception as e:
            log.error("Backtest Excel report generation failed: %s", e)
    else:
        log.warning("Zero trades recorded — Excel report skipped (nothing to analyse)")
