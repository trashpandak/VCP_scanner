...
def run_scan(symbols: list[str], benchmark: pd.DataFrame) -> list:
    ...
    for i, (sym, daily) in enumerate(cached_daily.items(), 1):
        if i % 100 == 0:
            log.info("  Scanning %d/%d — signals found: %d", i, len(cached_daily), len(signals))
        try:
    ...
    log.info("Scan complete: %d VCP signals found (%d errors)", len(signals), errors)
    return signals
...
if __name__ == "__main__":
    ...
    signals = run_scan(symbols, benchmark)
    _print_signals(signals)

    # Always generate the daily Excel report — independent of --save.
    # This is the fix for "daily scan not giving the scans in excel file":
    # previously NO excel output existed for the live scan at all, only
    # the backtest had an Excel report. --save only controlled whether
    # signals were written to the SQLite database.
    if signals:
        try:
            from vcp_daily_report import generate_daily_vcp_report
            report_path = generate_daily_vcp_report(signals)
            if report_path:
                print(f"\n  ✅ Daily Excel report saved: {report_path}\n")
        except Exception as e:
            log.error("Daily Excel report generation failed: %s", e)
